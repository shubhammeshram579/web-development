...
const AdoptionReq = () => {
    ...
    const handelBtn = (e) => {
      e.preventDefault();
      let  status = shelterData.map((item) => {
        if(item.status === false){
          item.status = true
        }
      });
      console.log(status)
      dispatch(adoptionReq(status))

    }

  return (
    <div style={{paddingTop:"100px",width:"80%",marginLeft:"16%", paddingBottom:"100px" ,display:"flex", flexWrap:"wrap", gap:"20px"}}>
        {shelterData.map((items, index) => (
      <div className="card1" style={{backgroundColor:"#fff" ,width:"20%" ,color:"#111" ,padding:"10px" ,borderRadius:"10px"}}>
        ...
        <div style={{display:"flex", alignItems:"center" ,justifyContent:"space-around",paddingTop:"20px",paddingBottom:"10px"}}>
            <button style={{border:"none",backgroundColor:"orange", color:"#fff",padding:"8px 20px",borderRadius:"10px"}}>Reject</button>
